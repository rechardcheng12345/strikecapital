...
export async function notifyAllInvestors(
  type: string,
  title: string,
  message: string,
  metadata?: Record<string, any>
      ...
      for (const pos of expiringPositions) {
        ...
        if (!existing) {
          await notifyAllInvestors(
            'expiry_alert',
            `Expiry Alert: ${pos.ticker} $${pos.strike_price} Put`,
            `Position expires in ${threshold.label} (${pos.expiration_date})`,
            { position_id: pos.id, threshold: threshold.label }
          );
        }
      }
    }
...
export function startExpiryAlertJob(intervalMinutes: number = 60): void {
...
export function stopExpiryAlertJob(): void {
  if (expiryInterval) {
