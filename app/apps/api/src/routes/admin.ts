...
router.get('/dashboard/stats', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const total_positions = parseInt(totalPosResult?.count as string || '0');

    // Open positions (OPEN + MONITORING)
    const [openPosResult] = await db('positions')
      .whereIn('status', ['OPEN', 'MONITORING'])
      .count('* as count');
    const open_positions = parseInt(openPosResult?.count as string || '0');
    ...
    const total_investors = parseInt(investorCount?.count as string || '0');
    ...
    const [expiringResult] = await db('positions')
      ...
      .count('* as count');
    const positions_expiring_soon = parseInt(expiringResult?.count as string || '0');
...
router.post('/investors', validate(createInvestorSchema), async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const [user] = await db('users')
      .insert({ email, password_hash, full_name, phone: phone || null, role: 'investor' })
      .returning('*');
...
router.put('/investors/:id', validate(updateInvestorSchema), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const user = await db('users').where({ id: req.params.id, role: 'investor' }).first();
    if (!user) throw new AppError('Investor not found', 404);
    ...
    if (invested_amount !== undefined || allocation_pct !== undefined) {
      ...
      const allocUpdates: any = {};
      if (invested_amount !== undefined) allocUpdates.invested_amount = invested_amount;
      if (allocation_pct !== undefined) allocUpdates.allocation_pct = allocation_pct;
...
router.post('/investors/:id/reset-password', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const user = await db('users').where({ id: req.params.id, role: 'investor' }).first();
    if (!user) throw new AppError('Investor not found', 404);
...
router.get('/pnl', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (startDate) pnlQuery = pnlQuery.where('record_date', '>=', startDate);
    ...
    const total_realized_pnl = records.reduce((sum: number, r: any) => sum + parseFloat(r.pnl_amount), 0);

    // Fetch positions for the period
    let posQuery = db('positions');
    if (startDate) posQuery = posQuery.where('created_at', '>=', startDate);
    const positions = await posQuery.orderBy('created_at', 'desc');

    // Total premium from positions in period
    const total_premium = positions.reduce((sum: number, p: any) => sum + parseFloat(p.premium_received || '0'), 0);
...
router.post('/pnl', validate(createPnlSchema), async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const [record] = await db('pnl_records')
      .insert({
        position_id,
...
router.post('/announcements', validate(announcementSchema), async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.put('/announcements/:id', validate(announcementSchema.partial()), async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.get('/risk/dashboard', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const ticker_concentration = (concentration as any[]).map((c: any) => ({
      ticker: c.ticker,
    ...
    (distanceToStrike as any[]).forEach((pos: any) => {
      const dist = parseFloat(pos.distance_pct || pos.distancePct || pos.distance || '0');
      if (dist < 3) rangeBuckets['0-3%']++;
      else if (dist < 5) rangeBuckets['3-5%']++;
      else if (dist < 10) rangeBuckets['5-10%']++;
      else if (dist < 20) rangeBuckets['10-20%']++;
      else rangeBuckets['20%+']++;
    });
...
router.get('/audit', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (action) { query = query.where('audit_logs.action', action as string); countQuery = countQuery.where('action', action as string); }
    if (entity_type) { query = query.where('audit_logs.entity_type', entity_type as string); countQuery = countQuery.where('entity_type', entity_type as string); }
    if (start_date) { query = query.where('audit_logs.created_at', '>=', start_date as string); countQuery = countQuery.where('created_at', '>=', start_date as string); }
    if (end_date) { query = query.where('audit_logs.created_at', '<=', (end_date as string) + ' 23:59:59'); countQuery = countQuery.where('created_at', '<=', (end_date as string) + ' 23:59:59'); }
...
router.get('/export/positions', async (req: Request, res: Response, next: NextFunction) => {
  try {
