...
router.put('/settings', authenticate, requireAdmin, validate(updateSettingsSchema), async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    let settings;
    if (existing) {
      [settings] = await db('fund_settings').where({ id: existing.id }).update(updates).returning('*');
    } else {
      [settings] = await db('fund_settings').insert({ ...updates }).returning('*');
    }
...
// GET /capacity
router.get('/capacity', authenticate, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const settings = await db('fund_settings').first();
