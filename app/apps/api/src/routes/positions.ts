  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (ticker) {
        ...
        countQuery = countQuery.where('ticker', (ticker as string).toUpperCase());
      }
      if (position_type) {
        query = query.where('position_type', position_type as string);
        countQuery = countQuery.where('position_type', position_type as string);
      }
    ...
    try {
      ...
      if (position_type === 'stock') {
        ...
        await notifyAllInvestors(
          'position_opened',
          `New Stock Position: ${ticker.toUpperCase()} (${stockShares} shares)`,
          `Stock position opened at $${stockCostBasis.toFixed(2)} cost basis.`,
      ...
      await notifyAllInvestors(
        'position_opened',
        `New Position: ${ticker.toUpperCase()} $${strike_price} Put`,
        `${contracts} contract(s) opened with $${premium_received.toFixed(2)} premium received.`,
        { position_id: position.id }
      );
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      await notifyAllInvestors(
        'position_resolved',
        `Position Resolved: ${position.ticker} $${position.strike_price} Put`,
        `Resolution: ${resolution_type}. Realized P&L: $${realized_pnl.toFixed(2)}.`,
        { position_id: parseInt(id, 10), resolution_type, realized_pnl }
      );
      ...
      if (resolution_type === 'assigned' && position.position_type !== 'stock') {
        ...
        [stockPosition] = await db('positions')
          .insert({
        ...
        await notifyAllInvestors(
          'position_opened',
          `Assignment: ${position.ticker} Stock (${stockShares} shares)`,
          `Option assigned. Stock position created at $${stockCostBasis.toFixed(2)} cost basis.`,
          { position_id: stockPosition.id, assigned_from: parseInt(id, 10) }
        );
      }
