...
router.get('/dashboard', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const resolved = parseInt(resolvedCount?.count as string || '0');
    const won = parseInt(wonCount?.count as string || '0');
    ...
    res.json({
      ...
      active_positions: parseInt(activeResult?.count as string || '0'),
      win_rate: winRate,
...
router.get('/positions/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.get('/pnl', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const now = new Date();
    if (period === '1m') startDate = new Date(now.getFullYear(), now.getMonth() - 1, now.getDate()).toISOString().split('T')[0];
    else if (period === '3m') startDate = new Date(now.getFullYear(), now.getMonth() - 3, now.getDate()).toISOString().split('T')[0];
    ...
    if (startDate) pnlQuery = pnlQuery.where('pnl_records.record_date', '>=', startDate);
    ...
    const totalRealized = records.reduce((sum: number, r: any) => sum + parseFloat(r.pnl_amount), 0);
...
router.get('/history', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // Summary stats
    const resolved = parseInt(count as string);
    const [wonCount] = await db('positions')
      .where('status', 'RESOLVED')
      .whereIn('resolution_type', ['expired_worthless', 'rolled'])
      .count('* as count');
    ...
    res.json({
      ...
      stats: {
        ...
        win_rate: resolved > 0 ? Math.round((parseInt(wonCount?.count as string || '0') / resolved) * 100) : 0,
      },
      pagination: {
        page, limit,
        total: resolved,
