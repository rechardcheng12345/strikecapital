...
function statusBadgeVariant(status: Position['status']): 'green' | 'yellow' | 'blue' | 'red' | 'gray' {
  const map: Record<string, 'green' | 'yellow' | 'blue' | 'red' | 'gray'> = {
    OPEN: 'green',
    MONITORING: 'yellow',
    ROLLING: 'blue',
    EXPIRY: 'red',
...
function RiskMetric({ label, value, icon, highlight }: RiskMetricProps) {
  return (
    <div
      className={`rounded-none border-2 p-4 transition-all duration-150 ${
        highlight
      ...
      <div className="flex items-center gap-2 mb-2">
        <div
          className={`p-1.5 rounded-none ${
            highlight ? 'bg-[#F06010]/10 text-[#F06010]' : 'bg-[#0D2654]/5 text-[#0D2654]'
          }`}
        >
          {icon}
        </div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
        {Array.from({ length: 6 }).map((_, i) => (
          <div key={i} className="rounded-none border-2 border-gray-200 bg-white p-4 space-y-2">
...
export function PositionDetailPage() {
  ...
  // Resolve form
  const [resolveForm, setResolveForm] = useState<ResolvePositionData>({
  ...
  // Roll form
  const [rollForm, setRollForm] = useState<CreatePositionData>({
    ticker: '',
    strike_price: 0,
    premium_received: 0,
    contracts: 1,
  ...
  const {
    ...
    error,
    refetch,
  } = useApiQuery<Position>({
    queryKey: ['position', positionId, isAdmin ? 'admin' : 'investor'],
    queryFn: () =>
      isAdmin ? positionApi.get(positionId) : investorApi.getPosition(positionId),
  ...
  return (
    <div>
      ...
      <button
        ...
        className="inline-flex items-center gap-1.5 text-sm font-medium text-[#0D2654]/60 hover:text-[#0D2654] transition-colors mb-4"
      >
      ...
      {isError && (
        <div className="mb-6">
          <ErrorAlert
            message={error?.message || 'Failed to load position.'}
            onRetry={() => refetch()}
          />
        </div>
      )}
      ...
      {position && (
        <div className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            {position.position_type !== 'stock' && position.expiration_date && (
              <div className="flex items-center gap-2 text-sm text-gray-500">
                <Calendar className="w-4 h-4" />
                Expires {formatDate(position.expiration_date)}
              </div>
            )}
            {position.position_type === 'stock' && position.shares && (
              <div className="flex items-center gap-2 text-sm text-gray-500">
          ...
          {/* ─── Admin actions ─────────────────────────── */}
          {isAdmin && position.status !== 'RESOLVED' && (
            <div className="flex flex-wrap gap-2">
              ...
              {position.position_type !== 'stock' && (
                <Button variant="outline" size="sm" onClick={initRollForm}>
              ...
              <Button
                variant="outline"
                size="sm"
                onClick={() => navigate(`/admin/positions/${positionId}/edit`)}
              >
            ...
            {position.position_type === 'stock' ? (
              <>
                ...
                <RiskMetric
                  ...
                  value={position.current_price && position.shares
          ...
          <Card className="rounded-none border-2 border-[#0D2654]/15">
            ...
            <CardBody>
              ...
              {position.notes && (
                <div className="mt-4 pt-4 border-t border-[#0D2654]/10">
              ...
              <CardBody>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-6 gap-y-4">
                  <DetailRow
                    ...
                    value={
                      position.resolution_type ? (
                        <span
                          className={
                            RESOLUTION_TYPE[position.resolution_type]?.color ?? 'text-gray-600'
                          }
                        >
                  ...
                  <DetailRow
                    ...
                    value={
                      position.realized_pnl != null ? (
                        <span
                          className={position.realized_pnl >= 0 ? 'text-green-600' : 'text-red-600'}
                        >
          ...
          {(position.rolled_from_id || position.rolled_to_id || position.assigned_from_id || position.assigned_to_id) && (
            <Card className="rounded-none border-2 border-blue-200 bg-blue-50/30">
              <CardBody>
                <div className="flex flex-col sm:flex-row gap-4">
                  {position.rolled_from_id && (
                    <div className="flex items-center gap-2 text-sm">
                      <ExternalLink className="w-4 h-4 text-blue-600" />
                      <span className="text-gray-500">Rolled from:</span>
                      <Link
                        to={`${isAdmin ? '/admin' : ''}/positions/${position.rolled_from_id}`}
                  ...
                  {position.rolled_to_id && (
                    <div className="flex items-center gap-2 text-sm">
                      <ExternalLink className="w-4 h-4 text-blue-600" />
                      <span className="text-gray-500">Rolled to:</span>
                      <Link
                        to={`${isAdmin ? '/admin' : ''}/positions/${position.rolled_to_id}`}
                        className="font-medium text-blue-600 hover:text-blue-800 underline"
                      >
                  ...
                  {position.assigned_from_id && (
                    <div className="flex items-center gap-2 text-sm">
                      <ExternalLink className="w-4 h-4 text-purple-600" />
                      <span className="text-gray-500">Assigned from option:</span>
                      <Link
                        to={`${isAdmin ? '/admin' : ''}/positions/${position.assigned_from_id}`}
                  ...
                  {position.assigned_to_id && (
                    <div className="flex items-center gap-2 text-sm">
                      <ExternalLink className="w-4 h-4 text-purple-600" />
                      <span className="text-gray-500">Assigned to stock:</span>
                      <Link
                        to={`${isAdmin ? '/admin' : ''}/positions/${position.assigned_to_id}`}
                        className="font-medium text-purple-600 hover:text-purple-800 underline"
                      >
          ...
          >
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setResolveForm((f) => ({
                      ...f,
                      resolution_type: e.target.value as ResolvePositionData['resolution_type'],
                    }))
                  }
              ...
              <Input
                ...
                type="number"
                step="0.01"
                value={resolveForm.realized_pnl ?? ''}
                onChange={(e) =>
                  setResolveForm((f) => ({
                    ...f,
          ...
          >
            <div className="space-y-4">
              ...
              <Input
                label="Ticker"
                value={rollForm.ticker}
                onChange={(e) => setRollForm((f) => ({ ...f, ticker: e.target.value.toUpperCase() }))}
              />
              ...
              <div className="grid grid-cols-2 gap-4">
                <Input
                  ...
                  type="number"
                  step="0.01"
                  value={rollForm.strike_price || ''}
                  onChange={(e) =>
                    setRollForm((f) => ({ ...f, strike_price: parseFloat(e.target.value) || 0 }))
                  }
                />
                <Input
                  ...
                  type="number"
                  step="0.01"
                  value={rollForm.premium_received || ''}
                  onChange={(e) =>
                    setRollForm((f) => ({
                      ...f,
              ...
              <div className="grid grid-cols-2 gap-4">
                <Input
                  ...
                  type="number"
                  min={1}
                  value={rollForm.contracts}
                  onChange={(e) =>
                    setRollForm((f) => ({ ...f, contracts: parseInt(e.target.value) || 1 }))
                  }
                />
                <Input
                  label="Expiration Date"
                  type="date"
                  value={rollForm.expiration_date}
                  onChange={(e) => setRollForm((f) => ({ ...f, expiration_date: e.target.value }))}
                />
              </div>
