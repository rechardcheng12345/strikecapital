...
import type { Position, PositionsResponse } from '../../api/client';
...
type StatusFilter = keyof typeof POSITION_STATUS | 'ALL';
...
export function AdminPositionsPage() {
  ...
  const { data, isLoading, isError, error, refetch } = useApiQuery<PositionsResponse>({
            ...
            <EmptyState
              ...
              description={
                activeTab === 'ALL'
                  ...
                  : `No positions with status "${activeTab}".`
              }
              action={
                <Link to="/admin/positions/new">
                  <Button variant="primary" className="bg-[#F06010] hover:bg-[#d9560e] rounded-none">
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`inline-flex items-center px-2.5 py-0.5 rounded-none text-xs font-medium ${
                          STATUS_BADGE_VARIANT[pos.status] || 'bg-gray-100 text-gray-800'
                        }`}
                      >
