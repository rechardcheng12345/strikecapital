...
function AnnouncementFormModal({ isOpen, onClose, announcement }: FormModalProps) {
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="w-full">
          ...
          <input
            ...
            className="block w-full px-3 py-2 border border-gray-300 rounded-none shadow-sm focus:outline-none focus:ring-2 focus:ring-[#F06010] focus:border-[#F06010] sm:text-sm"
          />
        </div>
        ...
        <div className="w-full">
          ...
          <textarea
            ...
            className="block w-full px-3 py-2 border border-gray-300 rounded-none shadow-sm focus:outline-none focus:ring-2 focus:ring-[#F06010] focus:border-[#F06010] sm:text-sm resize-y"
          />
        </div>
...
function DeleteConfirmModal({ isOpen, onClose, announcement }: DeleteModalProps) {
  const queryClient = useQueryClient();
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Delete Announcement" size="sm">
...
export function AnnouncementsPage() {
  ...
  const {
    ...
    error,
    refetch,
  } = useApiQuery<AnnouncementsResponse>({
  ...
  return (
    <div>
      ...
      {!isLoading && announcements.length === 0 && (
        <Card className="rounded-none border-2 border-[#0D2654]/20">
          <CardBody>
            <EmptyState
              icon={Megaphone}
              title="No announcements yet"
              description="Create your first announcement to keep investors informed."
              action={
                <Button
                  variant="primary"
                  onClick={() => setShowCreateModal(true)}
      ...
      {!isLoading && announcements.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {announcements.map((ann) => (
            <div
              key={ann.id}
              className={`bg-white border-2 rounded-none p-5 transition-all duration-150 ${
                ann.is_active
            ...
            >
              ...
              <div className="flex items-start justify-between gap-3 mb-3">
                <h3
                  className="text-base font-bold text-[#0D2654] line-clamp-1 flex-1"
                  style={{ fontFamily: 'Space Grotesk, sans-serif' }}
                >
                  {ann.title}
                </h3>
              ...
              <div className="flex items-center justify-between pt-3 border-t border-gray-100">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <button
                    ...
                    className="p-1.5 hover:bg-gray-100 rounded-none transition-colors disabled:opacity-50"
