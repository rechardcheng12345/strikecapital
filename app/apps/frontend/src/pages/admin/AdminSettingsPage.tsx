...
import type { FundSettings } from '../../api/client';
...
export function AdminSettingsPage() {
  const { data: settings, isLoading, isError, error, refetch } = useApiQuery<FundSettings>({
          ...
          <Card className="rounded-none">
            <CardHeader className="rounded-none">
              <div className="flex items-center justify-between">
                <h2
                  className="text-lg font-semibold text-[#0D2654]"
                  style={{ fontFamily: 'Space Grotesk, sans-serif' }}
                >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="md:col-span-2">
                  ...
                  <Input
                    ...
                    error={errors.fund_name?.message}
                ...
                <div>
                  ...
                  <Input
                    type="number"
                    step="0.01"
                    {...register('total_fund_capital', {
                      required: 'Total fund capital is required',
                      min: { value: 0, message: 'Must be positive' },
                      valueAsNumber: true,
                    })}
                    error={errors.total_fund_capital?.message}
                    className="rounded-none"
                  />
                </div>
                ...
                <div>
                  ...
                  <Input
                    type="number"
                    step="0.01"
                    {...register('max_capital_per_position', {
                      required: 'Max capital per position is required',
                      min: { value: 0, message: 'Must be positive' },
                      valueAsNumber: true,
                    })}
                    error={errors.max_capital_per_position?.message}
                    className="rounded-none"
                  />
                </div>
                ...
                <div>
                  ...
                  <Input
                    type="number"
                    step="0.1"
                    {...register('default_alert_threshold', {
                      required: 'Alert threshold is required',
                      min: { value: 0, message: 'Must be non-negative' },
                      max: { value: 100, message: 'Cannot exceed 100%' },
                      valueAsNumber: true,
                    })}
                    error={errors.default_alert_threshold?.message}
                    className="rounded-none"
                  />
                </div>
              </div>
            ...
            <CardFooter className="rounded-none">
              <div className="flex items-center justify-end gap-3">
                ...
                <Button
                  type="submit"
                  className="rounded-none bg-[#0D2654] hover:bg-[#0D2654]/90"
