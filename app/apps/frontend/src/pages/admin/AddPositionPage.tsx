...
import type { CreatePositionData } from '../../api/client';
...
export function AddPositionPage() {
  ...
  const [positionType, setPositionType] = useState<'option' | 'stock'>('option');
  ...
  const createMutation = useMutation({
    mutationFn: (data: CreatePositionData) => positionApi.create(data),
    onSuccess: (response) => {
      if (response.error) {
            ...
            <CardBody>
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                ...
                <div className="flex gap-2 mb-6">
                  <button
                    type="button"
                    onClick={() => setPositionType('option')}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <Input
                    label={isStock ? 'Cost Basis ($)' : 'Strike Price ($)'}
                    type="number"
                    step="0.01"
                    min="0.01"
                    placeholder="0.00"
                ...
                <div>
                  ...
                  <textarea
                    className="block w-full px-3 py-2 border border-gray-300 rounded-none shadow-sm focus:outline-none focus:ring-2 focus:ring-[#F06010] focus:border-[#F06010] sm:text-sm min-h-[80px] resize-y"
                ...
                <div className="flex justify-end gap-3 pt-2">
                  ...
                  <Button
                    type="submit"
                    variant="primary"
                    className="bg-[#F06010] hover:bg-[#d9560e] rounded-none"
        ...
        <div className="lg:col-span-1">
          <Card className="rounded-none sticky top-6">
            <CardHeader className="bg-[#F5F3EF]">
              <h2
                className="text-lg font-semibold text-[#0D2654] flex items-center gap-2"
                style={{ fontFamily: 'Space Grotesk, sans-serif' }}
              >
