...
import { investorApi, type PositionHistoryResponse } from '../../api/client';
...
export function PositionHistoryPage() {
  ...
  const {
    ...
    error,
    refetch,
  } = useApiQuery<PositionHistoryResponse>({
  ...
  return (
    <div>
      ...
      {isError && (
        <div className="mb-6">
          <ErrorAlert
            message={error?.message || 'Failed to load position history.'}
            onRetry={() => refetch()}
          />
        </div>
      )}
      ...
      <div className="bg-white rounded-none border-2 border-[#0D2654]/20 overflow-hidden">
        {isLoading ? (
          ...
          <EmptyState
            ...
            description="There are no resolved positions in your history yet."
          />
        ) : (
              ...
              <tbody>
                {positions.map((pos, idx) => {
                  const resConfig = pos.resolution_type
                    ? RESOLUTION_TYPE[pos.resolution_type as keyof typeof RESOLUTION_TYPE]
                    : null;
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        {resConfig ? (
                          <span className={`font-medium ${resConfig.color}`}>{resConfig.label}</span>
                        ) : (
