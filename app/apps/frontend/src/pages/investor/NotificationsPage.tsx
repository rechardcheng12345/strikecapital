...
import { investorApi, type NotificationsResponse } from '../../api/client';
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        >
          ...
          Notifications
          {unreadCount > 0 && (
            <span className="ml-2 inline-flex items-center justify-center px-2.5 py-0.5 rounded-none text-xs font-bold bg-[#F06010] text-white">
              {unreadCount}
            </span>
          )}
        </h1>
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-none border-2 border-[#0D2654]/20">
          <EmptyState
            icon={Bell}
        ...
        <div className="space-y-3">
          {notifications.map((notif) => {
            const typeConfig = NOTIFICATION_TYPE[notif.type as keyof typeof NOTIFICATION_TYPE];
            return (
              <div
                key={notif.id}
                className={`rounded-none border-2 bg-white p-4 transition-all duration-150 ${
                  notif.is_read
                    ? 'border-[#0D2654]/10'
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      {typeConfig && (
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-none text-xs font-medium ${typeConfig.color}`}>
                          {typeConfig.label}
                        </span>
                      )}
                      <h3
                        className={`text-sm font-semibold ${notif.is_read ? 'text-gray-600' : 'text-[#0D2654]'}`}
                        style={{ fontFamily: 'Space Grotesk, sans-serif' }}
                      >
                        {notif.title}
                      </h3>
                  ...
                  {!notif.is_read && (
                    <button
                      ...
                      className="shrink-0 text-xs text-[#F06010] hover:text-[#0D2654] font-medium transition-colors disabled:opacity-50"
                    >
