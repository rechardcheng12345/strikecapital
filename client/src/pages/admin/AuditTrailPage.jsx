...
function actionBadgeVariant(action) {
    ...
    if (action.includes('delete'))
        return 'red';
    if (action.includes('resolve') || action.includes('roll'))
        return 'yellow';
    return 'gray';
}
...
function TableSkeleton() {
    return (<div className="space-y-0">
      {Array.from({ length: 8 }).map((_, i) => (<div key={i} className="flex items-center gap-4 px-4 py-3 border-b border-gray-100">
...
export function AuditTrailPage() {
    ...
    const { data, isLoading, isError, error, refetch } = useApiQuery({
        ...
        queryFn: () => adminApi.getAuditTrail(page, LIMIT, action || undefined, undefined, startDate || undefined, endDate || undefined),
    });
    ...
    return (<div className="space-y-6">
      ...
      <Card className="rounded-none">
        <CardBody>
          <div className="flex flex-wrap items-end gap-4">
            ...
            {/* Reset */}
            {hasActiveFilters && (<Button variant="ghost" size="sm" onClick={handleFilterReset} className="rounded-none">
                Clear Filters
              </Button>)}
          </div>
        </CardBody>
      ...
      <Card className="rounded-none overflow-hidden">
        {isLoading ? (<TableSkeleton />) : logs.length === 0 ? (<EmptyState icon={Search} title="No audit logs found" description={hasActiveFilters ? 'Try adjusting your filters.' : 'No activity has been recorded yet.'} action={hasActiveFilters ? (<Button variant="outline" size="sm" onClick={handleFilterReset} className="rounded-none">
                  Clear Filters
        ...
        {/* Pagination */}
        {pagination && totalPages > 1 && (<div className="flex items-center justify-between px-4 py-3 border-t border-gray-200 bg-gray-50">
            ...
            <div className="flex items-center gap-1">
              <Button variant="outline" size="sm" className="rounded-none" disabled={page <= 1} onClick={() => setPage(1)}>
              ...
              <Button variant="outline" size="sm" className="rounded-none" disabled={page >= totalPages} onClick={() => setPage(totalPages)}>
