...
import { Users, Plus, Search, Pencil, Trash2, ChevronLeft, ChevronRight, Copy, Check, Eye, } from 'lucide-react';
import { adminApi, } from '../../api/client';
import { useApiQuery } from '../../hooks/useApiQuery';
import { Button, Input, Card, CardBody, Badge, Modal, ErrorAlert, Skeleton, EmptyState, } from '../../components/ui';
...
function TableSkeleton() {
    return (<div className="space-y-0">
      {Array.from({ length: 5 }).map((_, i) => (<div key={i} className="flex items-center gap-4 px-6 py-4 border-b border-gray-100">
...
function InvestorFormModal({ isOpen, onClose, investor }) {
    ...
    if (tempPassword) {
        return (<Modal isOpen={isOpen} onClose={onClose} title="Investor Created" size="md">
        <div className="space-y-4">
          ...
          <div className="bg-[#F5F3EF] border-2 border-[#0D2654]/20 rounded-none p-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={handleCopy} className="p-2 hover:bg-[#0D2654]/10 rounded-none transition-colors" title="Copy password">
                {copied ? (<Check className="w-5 h-5 text-green-600"/>) : (<Copy className="w-5 h-5 text-[#0D2654]"/>)}
              </button>
            </div>
          </div>
    ...
    return (<Modal isOpen={isOpen} onClose={onClose} title={isEdit ? 'Edit Investor' : 'Add Investor'} size="md">
      <form onSubmit={handleSubmit} className="space-y-4">
        {formError && (<div className="bg-red-50 border border-red-200 rounded-none p-3 text-sm text-red-700">
            {formError}
          </div>)}

        <Input label="Email" type="email" required disabled={isEdit} value={form.email} onChange={(e) => setForm((f) => ({ ...f, email: e.target.value }))} placeholder="investor@example.com" className="rounded-none"/>

        <Input label="Full Name" type="text" required value={form.full_name} onChange={(e) => setForm((f) => ({ ...f, full_name: e.target.value }))} placeholder="John Smith" className="rounded-none"/>

        <Input label="Phone" type="tel" value={form.phone || ''} onChange={(e) => setForm((f) => ({ ...f, phone: e.target.value }))} placeholder="+1 (555) 000-0000" className="rounded-none"/>

        <Input label="Allocation Amount" type="number" min={0} step="0.01" value={form.allocation_amount || ''} onChange={(e) => setForm((f) => ({ ...f, allocation_amount: parseFloat(e.target.value) || 0 }))} placeholder="100000" className="rounded-none"/>
...
function InvestorDetailModal({ isOpen, onClose, investor }) {
    return (<Modal isOpen={isOpen} onClose={onClose} title="Investor Details" size="lg">
      <div className="space-y-4">
...
function DeleteConfirmModal({ isOpen, onClose, investor }) {
    ...
    return (<Modal isOpen={isOpen} onClose={onClose} title="Delete Investor" size="sm">
      <div className="space-y-4">
        <p className="text-sm text-gray-600">
    ...
    return (<div>
      ...
      <div className="mb-4">
        <div className="relative max-w-md">
          ...
          <input type="text" placeholder="Search by name or email..." value={search} onChange={(e) => handleSearchChange(e.target.value)} className="w-full pl-10 pr-4 py-2 border-2 border-[#0D2654]/20 rounded-none bg-white text-sm focus:outline-none focus:border-[#F06010] transition-colors"/>
        </div>
      </div>
      ...
      <Card className="rounded-none border-2 border-[#0D2654]/20 overflow-hidden">
        {isLoading ? (<CardBody>
            <TableSkeleton />
          </CardBody>) : investors.length === 0 ? (<CardBody>
            <EmptyState icon={Users} title="No investors found" description={debouncedSearch
                ? 'Try adjusting your search terms.'
                : 'Get started by adding your first investor.'} action={!debouncedSearch ? (<Button variant="primary" onClick={() => setShowAddModal(true)} className="rounded-none gap-2">
                    <Plus className="w-4 h-4"/>
                    Add Investor
                  </Button>) : undefined}/>
          </CardBody>) : (<>
            ...
            {/* Table Rows */}
            {investors.map((inv) => (<div key={inv.id} className="grid grid-cols-1 md:grid-cols-[1fr_1.2fr_0.8fr_0.6fr_0.8fr_0.8fr_0.6fr] gap-2 px-6 py-3 border-b border-gray-100 hover:bg-[#F5F3EF]/50 transition-colors items-center text-sm">
                ...
                <div className="flex items-center justify-end gap-1">
                  <button onClick={() => setViewingInvestor(inv)} className="p-1.5 hover:bg-[#0D2654]/10 rounded-none transition-colors" title="View details">
                    <Eye className="w-4 h-4 text-[#0D2654]"/>
                  </button>
                  <button onClick={() => setEditingInvestor(inv)} className="p-1.5 hover:bg-[#F06010]/10 rounded-none transition-colors" title="Edit investor">
                    <Pencil className="w-4 h-4 text-[#F06010]"/>
                  </button>
                  <button onClick={() => setDeletingInvestor(inv)} className="p-1.5 hover:bg-red-100 rounded-none transition-colors" title="Delete investor">
      ...
      {/* Pagination */}
      {pagination && pagination.pages > 1 && (<div className="flex items-center justify-between mt-4 px-1">
      ...
      {/* Modals */}
      {showAddModal && (<InvestorFormModal isOpen={showAddModal} onClose={() => setShowAddModal(false)}/>)}

      {editingInvestor && (<InvestorFormModal isOpen={!!editingInvestor} onClose={() => setEditingInvestor(null)} investor={editingInvestor}/>)}

      {viewingInvestor && (<InvestorDetailModal isOpen={!!viewingInvestor} onClose={() => setViewingInvestor(null)} investor={viewingInvestor}/>)}

      <DeleteConfirmModal isOpen={!!deletingInvestor} onClose={() => setDeletingInvestor(null)} investor={deletingInvestor}/>
    </div>);
}
